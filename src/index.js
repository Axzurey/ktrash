...
function GetPosts() {
	RedditApi.getSubreddit('dankmemes').getHot({limit : 100}).map(post => post).then(RedditAddPostMemes)
	RedditApi.getSubreddit('showerthoughts').getHot({limit : 100}).map(post => post).then(RedditAddPostShower)
	RedditApi.getSubreddit('darkjokes').getHot({limit : 100}).map(post => post).then(RedditAddPostJokes)
}
...
client.on('message', async message => {
	...
	if (message.content.toLowerCase() == "happy birthday to me") {
		print("bday")
		message.channel.send("Happy Birthday!!!!!")
	}
	if (message.content.toLowerCase() == "party time") {
		print("party")
		message.channel.send("WOOOOOOOOOOO")
	}

	if (message.content.toLowerCase() == "want to play a game?") {
		print("game")
		message.channel.send("depends")
	}

	if (message.content.toLowerCase() == "bj?") {
		print("game")
		message.channel.send("what a clown")
	}
	...
	if (command == "dbget") {
		...
		try {
			...
			let ToSend = `Data for ${args[0]} retrieved @Schema:Konta => ${UData}`
	...
	if (command == "dbcreate") {
		...
		try {
			...
			UData = await GetDataForUser(args[0])
			let ToSend = `Data for ${args[0]} created @Schema:Konta => ${UData}`
			Embed.setDescription(ToSend)
			Channel.send(Embed)
		}
		catch (err) {
